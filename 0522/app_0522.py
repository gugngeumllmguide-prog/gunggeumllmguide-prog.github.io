...
PATTERN_IMAGE_DIR = os.path.join(BASE_DIR, "../문양")
...
def load_pattern_candidates():
    ...
    if not os.path.exists(PATTERN_IMAGE_DIR):
        ...
        return _pattern_cache
    results = []
    for fname in os.listdir(PATTERN_IMAGE_DIR):
        if fname.lower().endswith((".jpg", ".jpeg", ".png")):
            stem = os.path.splitext(fname)[0]
            if "_" in stem:
                loc, name = stem.split("_", 1)
            else:
                loc, name = "", stem
            results.append({
                "name": name,
                "location": loc,
                "img_path": os.path.join(PATTERN_IMAGE_DIR, fname)
            })
    _pattern_cache = results
    print(f"[문양 로드] {len(results)}개 항목")
    return results



def find_best_pattern_match(img_pil, location, candidates):
    if not candidates:
        return None
    from PIL import Image as _Image

    filtered = [c for c in candidates if c.get("location") == location]
    pool = filtered if filtered else candidates

    ref_images_prompt = []
    for c in pool[:12]:
        try:
            ref_img = _Image.open(c["img_path"]).convert("RGB")
            ref_images_prompt.append((c["name"], ref_img))
        except:
            continue

    if ref_images_prompt:
        contents = [img_pil, f"현재 위치: {location}\n다음은 창덕궁 문양 참고 이미지들입니다."]
        for name, ref_img in ref_images_prompt:
            contents.append(f"[{name}]")
            contents.append(ref_img)
        contents.append("사용자 사진과 가장 유사한 항목의 이름만 정확히 답하세요. 없으면 '없음'.")
        try:
            result = client.models.generate_content(model=model_id, contents=contents)
            matched = result.text.strip().replace("*", "").replace("#", "")
            print(f"[이미지 매칭] {matched}")
            return matched if matched != "없음" else None
        except Exception as e:
            print(f"[매칭 오류] {e}")
            return None

    name_list = [c["name"] for c in pool]
    try:
        result = client.models.generate_content(
            model=model_id,
            contents=[img_pil, f"현재 위치: {location}\n다음 창덕궁 문양 목록 중 사진과 가장 유사한 것의 이름만 답하세요:\n{chr(10).join(name_list)}\n없으면 '없음'."]
        )
        matched = result.text.strip().replace("*", "").replace("#", "")
        return matched if matched != "없음" else None
    except Exception as e:
...
def search_element_kb(query="", location="", element="", top_k=3):
    ...
    for item in kb:
        ...
        if element:
            if element == name:
                score += 10
            elif element in name or name in element:
                score += 7
            if element == category:
                score += 10
            elif element in category or category in element:
                score += 8
            if element in keywords_text:
                score += 7
            elif any(element in kw or kw in element for kw in keywords_text.split()):
                score += 5
...
def build_context(query, kb, location="", element=""):
    if element:
        tokens = element.replace(",", " ").replace("/", " ").split()
        context_set = []
        seen = set()
        for token in tokens:
            for ctx in search_element_kb(query=query, location=location, element=token, top_k=3):
                if ctx not in seen:
                    context_set.append(ctx)
                    seen.add(ctx)
        if not context_set:
            context_set = search_element_kb(query=query, location=location, element=element, top_k=3)
        context_list = context_set
    else:
...
def classify_user_intent(query, kb_topics, has_image=False):
    if has_image:
        return "RAG_apply"
...
def ask():
    ...
    if image_b64:
        try:
            ...
            is_pattern_query = any(
                kw in question for kw in ["무늬", "문양", "뭐야", "뭐지", "이게", "이거"]) or not question.strip()

            if is_pattern_query:
                candidates = load_pattern_candidates()
                matched_name = find_best_pattern_match(img_pil, location, candidates)
                if matched_name:
                    image_description = matched_name
                    print(f"[크롤링 매칭 결과] {image_description}")
                else:
                    basic = client.models.generate_content(
                        model=model_id,
                        contents=[img_pil, f"현재 위치는 '{location}'입니다. 보이는 건축 요소나 장식 명칭을 한 단어로만 답하세요."]
                    )
                    image_description = basic.text.strip().replace("*", "").replace("#", "")
            else:
                basic = client.models.generate_content(
                    model=model_id,
                    contents=[img_pil, (
                        f"현재 위치는 '{location}'입니다. "
                        "이 사진에서 보이는 구체적인 건축 요소나 장식 명칭을 한 단어 또는 짧은 명사구로만 답하세요. "
                        "예: 사자문, 연꽃무늬, 청기와, 봉황, 수막새, 귀면형부조, 해태상 등. "
                        "'무늬'나 '문양' 같은 추상적 단어 대신 구체적 명칭을 사용하세요."
                    )]
                )
                image_description = basic.text.strip().replace("*", "").replace("#", "")
    ...
    kb_names = set(kb.keys())
    local_kb = load_local_element_kb()
    local_names = {item.get("name", "") for item in local_kb}
    local_categories = {item.get("category", "") for item in local_kb}
    local_keywords = {kw for item in local_kb for kw in item.get("keywords", [])}
    all_known = kb_names | local_names | local_categories | local_keywords

    image_in_kb = any(
        token in all_known
        for token in (image_description or "").replace(",", " ").split()
        if len(token) >= 2
    )
    intent = classify_user_intent(combined_question, kb.keys(), has_image=image_in_kb)
